...
export default {
  ...
  computed: {
    ...
    extraServices() {
      return this.proposal.extraServices[this.category].filter((item) => !item.added && item.price);
    },
    tax() {
    ...
    totalPriceOfProposal() {
      let totalPrice = 0;
      let services = this.proposal.additionalServices.length
        ? this.proposal.bookedServices
        : Object.keys(this.proposal.costServices);
      services.map((serviceCategory) => {
        const sumOfService = this.proposal.costServices[serviceCategory].reduce((s, item) => {
