<template>
  <div class="vtimeselector">
    <div class="vtimeselector_content">
      <input type="text"
           readonly="readonly"
           autocomplete="off"
           class="vtimeselector__input_1"
           :value="timeFormated('displayFormat','time')"
           :id="id"
           :disabled="disabled"
           :required="required"
           :name="name"
           :class="{'vtimeselector__input_1--is-open': !pickerState.isClosed_1}"
           @click="togglePicker_1">
      <input type="text"
           readonly="readonly"
           autocomplete="off"
           class="vtimeselector__input_2"
           :value="timeFormated('displayFormat','period')"
           :id="id"
           :disabled="disabled"
           :required="required"
           :name="name"
           :class="{'vtimeselector__input_2--is-open': !pickerState.isClosed_2}"
           @click="togglePicker_2">
    </div>
    ...
    <div class="vtimeselector__box_1" :class="{'vtimeselector__box_1--is-closed': pickerState.isClosed_1}">
    ...
    <div class="vtimeselector__box_2" :class="{'vtimeselector__box_2--is-closed': pickerState.isClosed_2}">
...
export default {
  ...
  methods: {
    ...
    timeFormated (type, format) {
      ...
      if (type === 'displayFormat' && !this.displayFormat) {
        ...
        if (format==="time")
          ...
          return !this.h24 ? (this.picker.hour >= 12 ? ' AM' : ' PM' ) : ''
      }
    ...
    */
    selectTime (type, el, e) {
      ...
      if (!this.disabled && !this.getState(type, 'disable', el)) {
        ...
        * @type {Date}
        */
        console.log("emit1", `selected${type.charAt(0).toUpperCase() + type.slice(1)}`, this.picker[type]);
        ...
        */
        ...
        //   if (type === "ampm") {
        //     this.$emit('formatedTime', this.timeFormated('returnFormat', "period"));
        //   } else  {
        //     this.$emit('formatedTime', this.timeFormated('returnFormat', "time"));
        //   }
        /**
    ...
    */
    togglePicker_1 () {
      if (!this.disabled) {
        this.pickerState.isClosed_1 = !this.pickerState.isClosed_1;
        /**
        * Emit opened|closed modal event
        * @event opened|closed
        * @type {null}
        */
        if (!this.pickerState.isClosed_2) {
          this.pickerState.isClosed_2 = true
        }
        this.pickerState.isClosed_1 ? this.$emit('closed', this.$el) : this.$emit('opened', this.$el);
      }
    },
    /**
    * Toggle Picker in order to open or close the select modal
    ...
    */
    togglePicker_2 () {
      if (!this.disabled) {
        this.pickerState.isClosed_2 = !this.pickerState.isClosed_2;
        /**
        * Emit opened|closed modal event
        * @event opened|closed
        * @type {null}
        */
        if (!this.pickerState.isClosed_1) {
          this.pickerState.isClosed_1 = true
        }
        this.pickerState.isClosed_2 ? this.$emit('closed', this.$el) : this.$emit('opened', this.$el);
      }
    },
    /**
    ...
    */
    close (e = null) {
      if (e === null) {
        !this.pickerState.isClosed_1 && this.togglePicker_1();
        !this.pickerState.isClosed_2 && this.togglePicker_2();
      } else if (!this.$el.contains(e.target) && this.$el !== e.target && !this.pickerState.isClosed_1) {
        this.togglePicker_1()
      } else if (!this.$el.contains(e.target) && this.$el !== e.target && !this.pickerState.isClosed_2) {
        this.togglePicker_2()
      }
    },
    /**
...
<style>
  ...
  .vtimeselector__input_1 {
    width: calc(70% - 15px);
    margin-right: 10px;
    box-sizing: border-box;
  }
  .vtimeselector__input_2 {
    width: 30%;
    box-sizing: border-box;
  }
  ...
  .vtimeselector__box_1--is-closed { display: none; }
  .vtimeselector__box_2--is-closed { display: none; }
